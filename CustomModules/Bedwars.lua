...
runcode(function()
    ...
    local TexturePack = Tabs["Utility"]:CreateToggle({
        ...
        ["Callback"] = function(Callback)
            Enabled = Callback
			if Enabled then
                local obj = game:GetObjects("rbxassetid://12483505466")[1]
                obj.Name = "Part"
...
runcode(function()
    local Enabled = false
    local Cham = Tabs["Render"]:CreateToggle({
        ["Name"] = "Cham",
        ["Callback"] = function(Callback)
            Enabled = Callback
            if Enabled then
...
getgenv().fillcolor = Color3.new(145, 108, 217) --Change fill color, no need to edit if using team color
